...
console.debug('[Habitaciones API INIT] baseURL =', apiClient.defaults.baseURL);
...
apiClient.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      ...
      console.debug(`[Habitaciones API REQUEST] ${tokenSource} token attached:`, token.substring(0, 20) + '...');
    } else {
      console.debug('[Habitaciones API REQUEST] No auth token found (neither admin nor web)');
    }

    console.debug('[Habitaciones API REQUEST]', (config.method || 'GET').toUpperCase(), `${config.baseURL || ''}${config.url || ''}`);
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      if (isAdminContext) {
        ...
        console.debug('[Habitaciones API] 401 Unauthorized - redirecting to /admin/login');
        globalThis.location.href = '/admin/login';
      } else {
        // Clear web auth
        localStorage.removeItem('authToken');
        localStorage.removeItem('authUser');
        console.debug('[Habitaciones API] 401 Unauthorized - redirecting to /login');
