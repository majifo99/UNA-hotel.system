...
import { ValueBlock, HISTORIAL_CONFIG } from "../components/Historial/HistorialComponents";
  ...
  const renderRows = () => {
    ...
    return rows.map((h: any, idx: number) => (
      <tr key={h.id_historial_limp ?? `${getRoomNumber(h)}-${idx}`} className="align-top odd:bg-slate-50/50 hover:bg-slate-100/60 transition-colors break-inside-avoid">
        ...
        <td className="px-4 py-2 whitespace-nowrap text-slate-700">{formatDatetime(asPrimitiveString(h.fecha))}</td>
