...
import { limpiezaWebSocketService } from '../services/websocketService';
import { toast } from 'sonner';
import type {
  NotificationContextType,
  StoredNotification,
  LimpiezaNotification,
} from '../types/notifications';

const NotificationContext = createContext<NotificationContextType | undefined>(undefined);

/**
 * Provider para gestionar notificaciones de limpieza en tiempo real
 *
 * Este provider:
 * - Conecta al WebSocket de Laravel Reverb
 * - Escucha eventos de nuevas limpiezas asignadas
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('limpieza_notifications');
    if (stored) {
      try {
        ...
        localStorage.removeItem('limpieza_notifications');
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (notifications.length > 0) {
      localStorage.setItem('limpieza_notifications', JSON.stringify(notifications));
    }
  }, [notifications]);
  ...
  useEffect(() => {
    try {
      const echo = limpiezaWebSocketService.getEcho();

      // Suscribirse al canal de limpiezas
      const channel = echo.channel('limpiezas');

    const handleNotification = (event: LimpiezaNotification) => {
      console.log('🔔 Nueva notificación de limpieza recibida:', event);
    ...
    console.log('👂 Escuchando en canal "limpiezas" el evento ".NuevaLimpiezaAsignada"');
    channel.listen('.NuevaLimpiezaAsignada', handleNotification);
    ...
    window.addEventListener('test-limpieza-notification', handleTestNotification);
    ...
    if (echo.connector?.pusher) {
      ...
      return () => {
        ...
        echo.leaveChannel('limpiezas');
        window.removeEventListener('test-limpieza-notification', handleTestNotification);
        // NO desconectar aquí para evitar problemas con React Strict Mode
        // limpiezaWebSocketService.disconnect();
      };
    }
   ...
   */
  const addNotification = useCallback((notification: LimpiezaNotification) => {
    const newNotification: StoredNotification = {
      ...
      limpieza_id: notification.data.id,
