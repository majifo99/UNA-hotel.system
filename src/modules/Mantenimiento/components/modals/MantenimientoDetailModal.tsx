...
import { X, BadgeCheck, UserCheck, Wrench, CalendarDays, Clock } from "lucide-react";
...
// ---- Helper descripción habitación (robusto con distintos campos posibles) ----
...
export default function MantenimientoDetailModal({ open, mantenimientoId, onClose }: Props) {
  ...
  // (Opcional) Historial demo — puedes reemplazarlo con el real cuando lo tengas
  const history = [
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 id="mant-dialog-title" className="text-white text-lg font-semibold leading-tight">
                  {titulo}
                </h2>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {item?.fecha_final && (
                <span className="inline-flex items-center gap-1 rounded-full border border-white/30 bg-white/10 px-2 py-0.5 text-xs font-medium text-white/90">
                  ...
                  <span>Finalizado: {fmtDateTime(item.fecha_final)}</span>
                </span>
              )}
              <button
                type="button"
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6">
                  ...
                  <div className={cn(card, "md:col-span-3")}>
                    ...
                    <div className="mt-2">
                      <Pill tone={estadoNombre?.toLowerCase() === "completado" ? "green" : "rose"}>
                        {estadoNombre}
                      </Pill>
                    </div>
